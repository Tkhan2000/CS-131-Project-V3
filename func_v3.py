...
class FunctionManager:
  def __init__(self, tokenized_program):
    ...
    self.capture_list = [[] for x in tokenized_program] # list of captures for each line
  ...
  # Create a FuncInfo object in func_cache (Used when a function variable is assigned)
  def create_function(self, func_name, func_info):
    self.func_cache[func_name] = func_info

  def set_lambda(self, args, line_num, captures = []):
  ...
  def _cache_function_parameters_and_return_type(self, tokenized_program):
    ...
    for line_num, line in enumerate(tokenized_program):
      ...
      # Include Return Types for lambdas
